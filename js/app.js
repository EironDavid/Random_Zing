(function () {
  ...
  const screens = [1, 2, 3, 4].map((n) => $('screen-' + n));
  ...
  function showScreen(num) {
    ...
    if (num === 2) replayPoemAnimation();
    if (num === 1) resetEnvelope();
    document.body.dataset.screen = String(num);
  }

  function goToScreen(num) {
    if (num < 1 || num > 4 || num === currentScreen) return;
    showScreen(num);
  }

  function goBack() {
    if (currentScreen > 1) {
      if (currentScreen === 4) resetInvite();
  ...
  function spawnBlooms() {
    const spots = [8, 15, 85, 20, 12, 45, 78, 50, 25, 80, 50, 70];
  ...
  btnMore.addEventListener('click', () => goToScreen(3));
  btnAsk.addEventListener('click', () => goToScreen(4));
